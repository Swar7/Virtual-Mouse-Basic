        ...
        if results.multi_hand_landmarks != None:
            for handLandmarks in results.multi_hand_landmarks:
                for point in mp_hands.HandLandmark:
                    ...
                    try:
                        Distance_x = sqrt((ringfingertip_x - middlefingertip_x) ** 2 + (ringfingertip_x - middlefingertip_x) ** 2)
                        Distance_y = sqrt((ringfingertip_y - middlefingertip_y) ** 2 + (ringfingertip_y - middlefingertip_y) ** 2)
                        if Distance_x < 5 or Distance_x < -5:
                            if Distance_y < 5 or Distance_y < -5:
                                click = click + 1
                                if click % 2 == 0:
                    ...
                    try:
                        Distance_x = sqrt((middlefingertip_x - ringfingertip_x) ** 2 + (middlefingertip_x - ringfingertip_x) ** 2)
                        Distance_y = sqrt((middlefingertip_y - ringfingertip_y) ** 2 + (middlefingertip_y - ringfingertip_y) ** 2)
                        if Distance_x < 5 or Distance_x < -5:
                            if Distance_y < 5 or Distance_y < -5:
                                click = click + 1
                                if click % 2 == 0:
